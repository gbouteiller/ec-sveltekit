<div class="flex h-full w-full flex-col md:flex-row">
  <Card intent="light" title="Me contacter" class={INFOS}>
    <div class="flex items-center gap-4 md:flex-col">
      <div class="w-full max-w-md">
        {#each social as item, i}<ContactItem {...item} class={CONTACT_ITEM} />{/each}
      </div>
  ...
  <Card intent="primary" class={{BASE: 'flex-1 overflow-y-auto before:mt-auto after:mb-auto'}}>
    <Form.Root form={data.form} method="POST" schema={zData} {options} let:config let:delayed class="flex w-full max-w-3xl flex-col gap-6">
      ...
      <Form.Button disabled={delayed} class="self-end">
        {#if delayed} <Icon icon="mdi:loading" class="mr-2 h-6 w-6 animate-spin" />{/if}
        Envoyer
      </Form.Button>
    </Form.Root>
    <Toaster />
